...
import StripeElementPayment from "./Payment.vue";
...
export default {
  ...
  data: () => ({
    ...
    paid: false,
    error: false,
    pk: process.env.VUE_APP_STRIPE_PUBLISHER_KEY,
    elementsOptions: {
      appearance: {
        theme: "stripe",
      }, // appearance options
  ...
  methods: {
    ...
    createPayment() {
      ...
      return Promise.resolve(this.$store.state.payment)
        .then((payment) => payment?.id && payments.deleteItem(payment))
        .then(() =>
          payments
            .createItem({
              ...
              /* custom: {
                ...
                fields: {
                  getPaymentMethodsRequest: JSON.stringify({
                    ...
                    amount: amountToPSP(this.amount),
                  }),
                },
              },*/
            })
    ...
    onMount() {
      ...
      this.createPayment()
        .then((payment) => {
          ...
          this.elementsOptions.clientSecret = payment.interfaceId;
          /* const configuration = {
        ...
        .catch((error) => {
          ...
          console.warn("Error in Stripe", error);
          this.error = true;
        });
    },
    pay() {
      ...
      this.loading = false;
      this.paid = true;
      this.$emit("card-paid", this.paymentId);
      this.$store.dispatch("setPayment", undefined);
    },
  },
  mounted() {
    this.onMount();
